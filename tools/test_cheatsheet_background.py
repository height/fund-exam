...
def run():
    with sync_playwright() as p:
        for engine, width in [('chromium', 1100), ('webkit', 320)]:
            ...
            page.get_by_role('button', name='笔记本', exact=True).click()
            ...
            def finish(label):
                ...
                result = {'items': [{'sourceIds': [n['id']], 'title': label, 'markdown': n['markdown']} for n in source['notes']]}
            ...
            for w in [320, 390, 1100]:
                ...
                assert box['y'] + box['height'] <= nav['y'] - 4
            ...
            page.reload()
            page.get_by_role('region', name='上次生成的小抄').wait_for()
            assert len(requests) == 1 and cached(page) == saved
