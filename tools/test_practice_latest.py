"""New practice rounds start unanswered; cumulative results and cursor survive reload."""
import json
...
with sync_playwright() as p:
    for engine in ['chromium', 'webkit']:
        ...
        expect(page.get_by_role('button', name='打开练习答题卡')).to_contain_text(f'7/{len(questions)}')
        ...
        page.reload()
        expect(page.locator('.practice-last-result')).to_have_count(0)
        expect(page.locator(f'[data-pick="{q["answer"]}"]')).to_be_enabled()
        expect(page.locator('.practice-history-status')).to_contain_text('最近答对')
        expect(page.locator('.opt.ok, .opt.bad, .opt.sel')).to_have_count(0)
        page.get_by_role('button', name='打开练习答题卡').click()
        expect(page.get_by_role('button', name='第 7 题，最近答对', exact=True)).to_have_attribute('aria-current', 'true')
        expect(page.get_by_role('dialog')).to_contain_text('本轮已答 0 题')
        page.get_by_role('dialog').get_by_role('button', name='关闭', exact=True).click()
        ...
        page.reload()
        expect(page.locator('.practice-last-result')).to_have_count(0)
        expect(page.locator(f'[data-pick="{wrong}"]')).to_have_class('opt')
        expect(page.locator('.practice-history-status')).to_contain_text('最近答错')
        expect(page.locator('.opt:enabled')).to_have_count(4)
        saved = page.evaluate(READ, q['id'])
        ...
        browser.close()
        print(engine, 'PASS: fresh rounds, correct/wrong retry, chapter meter and preserved cursor/history', flush=True)
