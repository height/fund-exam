...
def run():
    ...
    with sync_playwright() as p:
        ...
        expect(page.locator('.fl-topic-row')).to_have_count(14)
        expect(page.locator('.fl-practice-list .fl-course-row')).to_have_count(6)
        page.screenshot(path='/tmp/formula-home.png', full_page=True)
        page.locator('.fl-practice-list summary').click()
        page.get_by_role('button',name='补习百分数、负数等数学基础（4 题）').click()
        ...
        for unit in COURSES:
            ...
            expect(page.locator('.fc-left > .fc-scene')).to_be_visible()
        ...
        page.locator('.fc-optional-scene summary').click()
        page.locator('input[aria-label="A 的资金占比"]').fill('60')
        expect(page.locator('.fc-optional-scene .fc-scene-result')).to_contain_text('14%')
        ...
        expect(page.locator('.fc-status')).to_have_text('已巩固')
        # Old reference URLs now land in the unified chapter catalog.
        page.goto(base+'#/formula?mode=reference')
        expect(page.locator('.fl-topic-row')).to_have_count(14)
        page.locator('.fl-topic-row').first.click()
        expect(page.locator('.fl-stages button')).to_have_count(3)
        page.get_by_role('button',name='下一步：例题解析',exact=True).click()
        page.get_by_role('button',name='下一步：检验理解',exact=True).click()
        expect(page.get_by_role('button',name='提交答案',exact=True)).to_be_disabled()
        page.get_by_role('button',name='B. 50 万',exact=True).click()
        page.get_by_role('button',name='提交答案',exact=True).click()
        expect(page.locator('.fc-feedback')).to_contain_text('答对了')
        expect(page.locator('.fl-check')).to_contain_text('不计入分步带练')
        ...
        expect(offline_page.locator('.fc-left > .fc-scene')).to_be_visible()
