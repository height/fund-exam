...
src/                  前端源码
  ...
  data/                 题库数据，由 tools/ 生成，别手改
    ...
    chapters.js           官方教材章序，由 tools/apply_chapters.py 生成
  styles.css            设计令牌 + 全部样式
public/               manifest 与图标，原样进 dist
tools/                Python 数据管线
  extract.py            从 materials/ 的备考 PDF 抽题 -> src/data/questions.json
  taxonomy.json         官方教材目录（科目一 8 章 / 科目二 18 章），章节归类的唯一白名单
  chapters.json         题目 id -> 官方章名，归类流程的产物，是事实来源
  apply_chapters.py     校验 chapters.json 并写回题库、按章序重排（--check 只校验）
...
- **章节练习**：首页入口，按官方教材目录列章（科目一 8 章 / 科目二 18 章），顶部切「练习 / 考试」。
  练习进入该章全部题目；考试从该章随机抽最多 30 题限时考，时长按每题 1.2 分钟折算（跟真考同节奏）。
  题库里还没有题的章会列出来但灰掉，免得以为覆盖全了。
...
章节按**官方教材目录**归类（科目一 8 章、科目二 18 章，见 `tools/taxonomy.json`）。
归类结果存在 `tools/chapters.json`（id -> 章名），由 `tools/apply_chapters.py` 校验后写回题库——
校验不通过就整体拒绝落盘：章名错了会让「章节练习」抽出一堆不相干的题，而这种错在界面上看不出来。
新抽出来的题若不在映射表里会标成「未归类」并在抽题日志里列出，不会被悄悄塞进某一章。
