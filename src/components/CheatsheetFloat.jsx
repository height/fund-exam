...
export default function CheatsheetFloat({ generation, go }) {
  ...
  return <aside ref={frame} className="cs-float" aria-label="小抄生成任务">
    <span className="cs-float-subject">{generation.subject}</span>
    <span className="sr-only" role="status" aria-live="polite">{error || label}</span>
    {busy ? <button className="cs-float-status" onClick={() => go('cheatsheet', { subject: generation.subject })} aria-label={`${label}，查看进度`}>
      ...
      {status === 'ready' ? <button className="cs-float-keep" title={error || '保留本次生成的小抄'} onClick={async () => { toast(await task.keep() ? '小抄已保留' : '保存失败，请重试保留或打开小抄下载') }}><Icon name="done" />{error ? '重试保留' : '保留'}</button>
        : <button className="cs-float-status" onClick={() => go('cheatsheet', { subject: generation.subject })} title={error}><Icon name="list" />生成未完成</button>}
      ...
      {status === 'ready' && <button className="cs-float-icon" aria-label="查看生成的小抄" title="查看生成的小抄" onClick={() => go('cheatsheet', { subject: generation.subject })}><Icon name="chevronRight" /></button>}
    </>}
  </aside>
}
