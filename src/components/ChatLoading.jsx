...
export default function ChatLoading({ received = 0 }) {
  ...
  return <details className="chat-work-status">
    ...
    <div className="chat-work-detail"><p>已携带选中文案、原文依据和本次对话。</p><p>{received ? `已接收 ${received.toLocaleString()} 字符，完整返回并校验后显示修改预览。` : '正在等待模型返回内容，可随时停止。'}</p></div>
  </details>
}
