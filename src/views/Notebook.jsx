...
export default function Notebook({ go, noteId, editRequested }) {
  ...
  return <>
    ...
    {error ? <div className="nb-empty" role="alert">笔记读取失败：{error}<button onClick={() => location.reload()}>重新加载</button></div>
      : loading ? <p role="status">正在打开笔记本…</p> : !notes.length ? <section className="nb-empty"><Icon name="list" size={32} /><h2>从一个想记住的考点开始</h2><p>长按选中文字，点“记笔记”后 AI 会自动整理。确认后才会加入笔记本。</p><button className="btn-pri" onClick={() => go('map')}>去知识图谱摘录 <Icon name="chevronRight" size={16} /></button></section>
