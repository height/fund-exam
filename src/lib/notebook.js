...
export function notePrompt(note) {
  return [
    ...
    '笔记默认展示结构：一句话核心知识（确有必要可多句话）＋表格（可选）＋公式（可选）＋图（可选），按此顺序排列。核心知识用普通段落，不默认拆成列表，也不加“定义、核心要点、总结”等栏目。只有来源明确给出的规则才能抽象表达；来源只有答案、例子或错误选项，无法支持通用规则时needsReview=true，简述缺少什么依据，不猜测补全。',
    ...
    '例：若解析明确给出“变动量=基数×变化率”，题设代入100和5%得到5，默认记“变动量=基数×变化率”，不记“本题结果为5”；若只有这组数字而没有规则依据，不反推通用公式。用户明确要求演示时可附最短示例，并标为示例，不能混同规则。',
