...
export function askAI(q, picked, signal) {
  return streamChat([
    ...
    '为降低阅读负担，必须按下面的 Markdown 层级输出，标题名称保持一致：' +
    '## 结论（先写正确答案与一句话考点）；' +
    '## 为什么（只讲支撑结论的业务逻辑，计算题在这里逐步列式）；' +
    '有干扰项或相近规则时再写 ## 易错辨析，并优先用短列表或小表格；' +
    '最后写 ## 记忆提示，只保留一个最有效的记法。' +
    '不要为了凑结构重复内容，每段不超过3句；只用加粗标记必须记住的术语、数字和法条信号词。' +
